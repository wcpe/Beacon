...
describe('Layout 顶栏化（FR-105 真机打磨：品牌上移整宽顶栏 + 状态条同栏）', () => {
  ...
  it('控制面状态条与品牌区同处整宽顶栏（不在侧边栏内）', async () => {
    ...
    const brandBtn = screen.getByText('Beacon').closest('button')
...
describe('Layout 侧栏结构（冻结顶/底，仅中间导航可滚）', () => {
  ...
  it('顶部品牌区与底部操作区冻结（shrink-0，不随导航滚动）', () => {
    renderLayout()
    // 品牌区容器（button 父级）冻结
    const brandBox = screen.getByText('Beacon').closest('button')?.parentElement
...
describe('Layout 品牌区可点跳可观测看板', () => {
  it('品牌区为可点 button 且含 logo（FR-123：连接小灯已移除）', () => {
    renderLayout()
    const brand = screen.getByRole('button', { name: '前往可观测看板' })
    // 品牌 logo（img src=/logo.svg）在按钮内
...
describe('Layout FR-123 品牌 logo + 动态标签标题', () => {
  it('品牌区显示 logo（img /logo.svg）', () => {
    renderLayout()
    const logo = document.querySelector('header img[src="/logo.svg"]')
    expect(logo).not.toBeNull()
  })

  it('品牌文案为「Beacon」（去「管理台」）', () => {
    renderLayout()
    expect(screen.getByText('Beacon')).toBeInTheDocument()
    expect(screen.queryByText('Beacon 管理台')).not.toBeInTheDocument()
  })

  it('document.title 随当前页设为「<页名> - Beacon」', () => {
    renderLayout('/configs')
    expect(document.title).toBe('配置中心 - Beacon')
  })

  it('未知路由 document.title 回退「Beacon」', () => {
    renderLayout('/totally-unknown-route')
    expect(document.title).toBe('Beacon')
  })
})
