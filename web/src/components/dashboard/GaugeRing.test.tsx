...
describe('GaugeRing', () => {
  ...
  it('正常等级（ok）环用绿色（#16a34a），占比对应角度', () => {
    const { container } = render(
      <GaugeRing icon={<span>i</span>} ratio={0.5} level="ok" label="L" valueText="v" />,
    )
  ...
  it('危险等级（danger）环用红色（#dc2626）', () => {
    const { container } = render(
      <GaugeRing icon={<span>i</span>} ratio={0.95} level="danger" label="L" valueText="v" />,
    )
    const ring = container.querySelector('[role="img"]') as HTMLElement
  ...
  it('注意等级（warn）环用琥珀色（#f59e0b）', () => {
    const { container } = render(
      <GaugeRing icon={<span>i</span>} ratio={0.75} level="warn" label="L" valueText="v" />,
    )
    const ring = container.querySelector('[role="img"]') as HTMLElement
  ...
  it('ratio=null（无上限）退化为 0 度（全轨道色），仅靠中心数值表达', () => {
    const { container } = render(
      <GaugeRing icon={<span>i</span>} ratio={null} level="ok" label="L" valueText="66" />,
    )
    const ring = container.querySelector('[role="img"]') as HTMLElement
    // 0deg 分界：已占比段为空
