// 运维设置单页单测（FR-62/FR-77，ADR-0048 拍平回单页）：
// 锁定行为——
// ① 6 域以一级 tab 呈现（默认健康检查 tab，切 tab 见对应组项）；
// ② 改 int 项保存以 updateSetting(key, "新值") 正确入参调用（值序列化为字符串）；
// ③ log.level 用下拉，改值保存以新枚举调用 updateSetting；
// ④ 后端 400 时把后端 message 经 toast 展示；
// ⑤ 值未变时保存按钮禁用；
// ⑥ 启动 / 安全项在 config.yml 的提示文案可见 + 页标题可见；
// ⑦【硬约束】跨域 tab 改两项时统一批量保存仍统观全部脏项（dirtyItems=2）；
// ⑧ update.* 更新相关项不在本页呈现（已挪到版本与更新页）。
...
// 设置项样例：覆盖 health（int）/ metric（bool）/ log（string，log.level 下拉特例）/ reverse-fetch（int），
// 另混入一项 update.*（应不出现在本页任何域 tab——已挪到版本与更新页）。
...
describe('SettingsPage 域 tab 呈现（FR-62，ADR-0048）', () => {
  it('页标题「运维设置」可见', async () => {
...
describe('SettingsPage 逐项保存（FR-62）', () => {
  ...
  it('后端 400 时展示后端中文 message', async () => {
    ...
    await userEvent.click(within(row).getByRole('button', { name: '保存' }))
    await waitFor(() => expect(showError).toHaveBeenCalledWith('非法值：必须为正整数'))
  })
})
...
describe('SettingsPage 批量保存 + 改动摘要（FR-77）', () => {
  ...
  it('跨域 tab 改两项后「保存全部变更（2）」对两项各调一次 updateSetting', async () => {
    renderPage(<SettingsPage />)
    // 改 health.ttl-sec 30→45（健康检查 tab）
    ...
    // 切到反向抓取 tab 改 reverse-fetch.max-file-bytes
