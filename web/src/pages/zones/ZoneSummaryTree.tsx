  ...
  return (
    <li>
      ...
      {zone.servers.length === 0 ? (
        <p className="ml-4 border-l py-0.5 pl-3 text-xs text-muted-foreground">无在册子服</p>
      ) : (
  ...
  return (
    <li>
      <div className="flex items-center py-0.5">
        <span className="text-sm font-semibold">大区 {group.group}</span>
...
export default function ZoneSummaryTree({ tree }: { tree: SummaryTree }) {
  if (tree.groups.length === 0) {
    return <p className="text-sm text-muted-foreground">无汇总数据</p>
  }
  return (
