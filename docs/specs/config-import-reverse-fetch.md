...
> 状态：已交付@v0.7.0（v0.7.0 后真机回归 → 方案 D 归真修复，见 [ADR-0028](../adr/0028-allow-hosting-agent-self-dir.md)）　·　关联 PRD：FR-39　·　分支：feature/config-import-reverse-fetch
...
- **自身目录碰撞（v0.7.0 后真机回归，已修复）**：agent 读盘必带自己的 `plugins/BeaconAgent/`，此前被控制面 `normalizePath` 保留目录闸整批拒为 `INVALID_PATH` → 反向抓取 100% 失效。方案 D 放开控制面拦截、自我保护下沉到 agent observe-only（不取/不写/不删自身目录但视为已收敛），一处后端改动同修 FR-38/FR-39，详见 [ADR-0028](../adr/0028-allow-hosting-agent-self-dir.md)。
