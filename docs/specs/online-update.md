...
> ⚠️ 本章（FR-96 launcher 方案）已被 **FR-119 / [ADR-0053](../adr/0053-single-binary-self-replace.md)** 取代为**单进程自替换 + 自动回滚**：去掉独立第二二进制 `beacon-launcher`，改由主进程在自身进程内 `rename` 让位自我替换、并新增「新版起不来自动回退」。下述 launcher / 退出码 `70` / `beacon-launcher` 设计**仅存档**，实际实现见 [docs/specs/single-binary-self-replace.md](single-binary-self-replace.md)。

> 状态：已被取代（存档）　·　关联 PRD：FR-96（本批）/ FR-97 / FR-99 / FR-101 / FR-102（后续补）　·　分支：feature/fr-96-launcher
...
主进程侧实现「在线更新核心」：按渠道查 GitHub Release 取最新版、与当前版本比对、下载本平台资产、SHA256 校验、原子落位 pending、触发主进程自替换重启（FR-119 取代原「以退出码 `70` 交还 launcher」）。仅控制面单二进制（含 `go:embed` 前端整体替换），agent jar 不在范围。HTTP 触发入口属 FR-99，本 FR 只提供可调用的更新服务方法 + 「就绪→触发自替换重启」这条接通，以服务级测试驱动。
...
- **自替换重启**：落位成功后经注入回调（`func()`）触发主进程优雅关停信号，`cmd/beacon` 的 `run()` 在关停后调自替换（`rename` 让位三步 + spawn 新进程），旧进程正常退出（`exit 0`）；取代原「`run()` 返回 `errRequestUpdateRestart` 哨兵 → `main()` 映射退出码 `70` 交还 launcher」路径（FR-119，[ADR-0053](../adr/0053-single-binary-self-replace.md)）。
...
- `go build ./...` + `go test ./...` 全绿；`internal/update` 单测覆盖：semver 高 / 低 / 相等 / rc 序 / dev 哨兵；SHA256 通过 / 失败；落位原子；failed 各分支（无资产 / 下载超限 / 校验不符）；自替换触发（更新就绪 → 优雅关停后调自替换重启，FR-119 取代原退出码交还）。
- 以 `httptest` mock release server 跑「查 → 下载 → 校验 → 落位 → 请求重启」服务级链路（不引真网络）。
- 完整「真触发 → 换二进制 → 主进程自替换重启 → 报新版」需 HTTP 触发（FR-99）+ Win/Linux 真机 → 待真机验（FR-119）。
