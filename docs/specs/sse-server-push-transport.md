...
- [x] agent-core：`StreamTransport`/`StreamEvent` 抽象（`transport/StreamTransport.kt`）+ `SseFrameParser`/`StreamEventTypes`（`stream/`）+ `AgentLifecycle` SSE 监听循环（注入 streamTransport 时取代三条长轮询）+ 重连对账。
