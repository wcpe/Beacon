...
>
> **后续演进（2026-07-29）**：本文记录 P1 已交付契约；§4.5/§5 的单一 `name` 与直接 DELETE 语义不再驱动新增实现。FR-205 将 namespace 拆为不可变 `code` + 可变 `displayName`，FR-216/218 将清理收敛为“先归档/可恢复，再经审批永久逻辑删除并保留不可复用墓碑”。新旧契约切换前必须保持兼容并失败关闭，禁止补建无审批的直删旁路。
