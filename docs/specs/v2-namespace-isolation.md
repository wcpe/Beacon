...
> 状态：草拟 · 关联 FR：FR-142 · 阶段：P3（0.23.x）
...
4. **`agent_ops` 指令白名单**：哪些 agent 指令支持跨域由各域规格逐条声明、默认不支持——P3 落地时该能力可能是「有枚举无消费者」的空集，是否推迟 `agent_ops` 枚举到有真实消费者时再加，待拍板（当前保留以完整覆盖 PRD 三禁令）。
