...
- **后端子服可达性**：配置的后端子服 up / 总数计数（经 **TCP 连接探测**——端口可连即 up、连接被拒 / 超时即 down，[ADR-0035](../adr/0035-backend-reachability-tcp-connect.md) 取代原 `ServerInfo.ping`，对不应答 status ping 的代理后端不误判）。
- **到各后端的平均延迟**：可达后端 **TCP 连接 RTT** 的平均毫秒（无可达后端时为不可用哨兵 -1）。
...
- bungee 壳 `BungeeProxyMetricsCollector` 采 BC 指标：在线连接数（`ProxyServer.getOnlineCount`）、线程 / 运行时长（core MXBean）、后端可达性 + 延迟（取 `ProxyServer.getServers()` 各后端 `socketAddress` 交 core `TcpBackendProbe` 并发 **TCP 连接探测**，有界等待汇总 up/total/平均延迟，[ADR-0035](../adr/0035-backend-reachability-tcp-connect.md)）。
- **不碰 MC 主线程**：采集在既有 async 上报线程内完成；后端 TCP 连接在独立守护线程池（`beacon-backend-probe`）并发执行，用 `CountDownLatch` 有界等待汇总，绝不阻塞主线程；超时 / 连接失败按 down 计、延迟不可用回退 -1（守架构不变量 #5）。
...
- agent BC 采集（含后端 **TCP 连接探测**）**不阻塞 MC 主线程**。
...
- **后端延迟 / 可达性为近似值**：**TCP 连接 RTT**（[ADR-0035](../adr/0035-backend-reachability-tcp-connect.md)，取代原 `ServerInfo.ping`）受网络抖动与有界等待超时影响，仅供趋势 / 概览参考，不作精确 SLA 依据；同 LAN 后端连接 RTT 常为亚毫秒（整数毫秒可能为 0）。
