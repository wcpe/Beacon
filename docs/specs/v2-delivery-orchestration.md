...
> 状态：已交付@v0.30.0 · 关联 FR：FR-162, FR-165, FR-166, FR-167, FR-168, FR-171 · 阶段：P9（0.29.x，v0.30.0 收口）
>
...
>
> **后续演进（2026-07-29）**：本文记录已交付 ChangeOrder 领域状态机；FR-207/211 交付后，统一审批内核通过 adapter 授权既有状态转换，允许 human 创建人本人审批，并把“批准 + 公开 start”收敛为“批准并由持久执行器自动执行”。正文的强制职责分离与二段启动仅作历史追溯，不得再叠加成第二套审批；变更单领域状态、灰度、熔断和回滚不被通用审批替代。
...
- 管到生效为止：批次 = 推送 → 触发生效（重启 / 配置热重载 / 仅推送）→ 观察窗 → 人工放行；`hot_reload` 只热更 V2 配置工件，普通文件与 JAR 仅落盘；「关了没起来」判生效失败并计入熔断。
...
- 生效编排：restart / hot_reload / push_only 三种生效方式；`hot_reload` 仅触发 V2 配置工件回调，普通文件与 JAR 只完成落盘；重启 = agent 优雅关服 + 宿主自启拉起，超时未回归判失败；观察窗健康数据供推进门展示与熔断判定。
...
17. hot_reload：仅配置工件触发配置热更回调且回执驱动状态；普通文件与 JAR 只落盘、不触发插件框架重载；含 .jar + hot_reload 组合在组单时有显式警告，并指引改用 restart。
