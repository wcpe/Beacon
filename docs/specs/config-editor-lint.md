...
- 校验规则：
  ...
  - `format=yaml`：用 **js-yaml**（`load`）做完整 YAML 1.2 解析（锚点 / 别名 / 多文档 / 深层流式集合皆覆盖）；解析抛 `YAMLException` 时取 `mark.line`（0 起换算 1 起）+ `reason` 作行内错误。引入 `js-yaml` 依赖经用户确认（规则 15）。
...
- **YAML 用 js-yaml 完整解析**（用户已确认加依赖）：覆盖锚点 / 别名 / 多文档 / 深层流式结构等完整 YAML 1.2 语法；单文档用 `load`（配置文件通常单文档，够用且更快）。仍以 FR-27 服务端 schema 校验为权威闸，前端 lint 是前置体验。
