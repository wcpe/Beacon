...
> 状态：草拟（**已按评审重写**）　·　关联 PRD：FR-229（增强 FR-157 / FR-89）　·　分支：feature/alert-batch-handle
...
- [ ] 术语收敛方案定稿（`acknowledged` / `resolved` 与"已读 / 已处理"的对应）
- [ ] 批量处理端点（按筛选，非仅当前页）+ 服务层
- [ ] 观测范围 guard 接入
- [ ] 批量审计
- [ ] 前端：筛选级批量入口 + 二次确认 + 文案统一
- [ ] 文档同步：PRD 状态、API、ADR-0064 引用、CHANGELOG
