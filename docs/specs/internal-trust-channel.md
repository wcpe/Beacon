...
> 状态：开发中·真机验收通过（待发版）　·　关联 PRD：FR-222　·　依赖：FR-219
...
### 6.1 真机验证记录（2026-09-21，隔离 Beacon 实例 :18999）

| # | 结果 | 证据 |
|---|---|---|
| 1 | ⚠️ 未验 | 开关关闭态下的 `pending` 落库未实调（需另一实例 + 关闭开关组合） |
| 2 | ✅ 通过 | 强随机 token + 开关开启，携 `X-Beacon-Token` 注册 `prod/fr222-machine-1` → 响应回带 `machineRegistered:true`、`identityId:c103be48-…`、`boundAt:2026-09-21T02:53:54Z`（**直接 active 并绑定**，无审批票据） |
| 3 | ✅ 通过 | 开关开启 + `agent-token` 仍为出厂默认值 → **启动期 fail-fast**：`配置校验失败: 开启 allow-machine-register 时 agent-token 必须改为强随机值（当前为留空或出厂默认值）`，退出码 1 |
| 4 | ✅ 通过 | 缺 token → `401 UNAUTHORIZED`（`缺少或非法的 token`）；错 token → `401 UNAUTHORIZED`，二者一致 |
| 5 | ⚠️ 未验 | 审计页核验未做（`identity.machine_registered` 落库由集成测试覆盖） |
| 6 | ⚠️ 未验 | 关闭态审计文案未核验 |
| 7 | ✅ 通过 | 既有 agent 注册流程未受影响（v1 注册端点对无 token 请求的鉴权行为逐字不变） |
| 8 | ⚠️ 未验 | 分配/换区审批要求未复测（该路径本次未改动） |

> 关键安全边界（弱 token fail-fast、受信/非受信分流、401 一致性）已真机确认；未验项集中在审计页核验与关闭态组合，属**下一轮补验范围**。真机验证用的强随机 token 建议轮换。
