...
- **未配 / 无命中 → 不设默认服 + WARN（绝不静默落错）**：BC 未配 home-zone、或所配 zone 在 Beacon 未设默认入口、或默认入口当前不在线/未注入时，**不设任何默认服**并打一条带 group/zone 上下文的中文 WARN（明确告知运维去 Beacon 配默认入口）。玩家此时遇 BungeeCord 原生「无默认服」拒绝（明确信号），**绝不被静默落到非大厅服**（跳过大厅有风险）。**不**回退到「首个在线 bukkit」。
- **可观测**：agent 注入子服地址 + 设默认服打 INFO 中文日志；选不出默认服打 WARN（去重，不每轮刷屏）。

- 范围内：zone 级默认入口（每 zone 唯一）的 CRUD + 校验 + 审计 + 下发；BC agent 设 BungeeCord 默认/fallback 服 + home-zone 选择 + 选不出时不设 + WARN；INFO/WARN 日志。
...
- **agent-core**：
  ...
  - `ProxyServerDirectorySyncer` 在注入子服后，**只**按「home-zone 在 Beacon 显式配置的在线默认入口」算出默认服 serverId 调 `directory.setDefaultServer(...)`；选不出（返回 null）→ 不设默认服 + 打一条去重 WARN。选择逻辑为可单测的纯逻辑（输入实例列表 + home-zone，输出默认服 serverId 或 null）。
  - 新增 `AgentSettings.proxy`（`ProxySettings{ homeGroup, homeZone }`），`AgentBootstrap` 读 `proxy.home-group` / `proxy.home-zone`（默认空串 = 未配 → 不设默认服 + WARN）。
...
- 不在 MC 主线程阻塞 IO（沿用既有 directory sync async 循环，WARN 也在该 async 循环里打）。控制面不可用 / 选不出默认入口 → 不设默认服 + WARN，绝不静默落任意服。
...
- agent 单测：home-zone 命中默认入口 → 选该 serverId 设默认服（不告警）；home-zone 未配 → 不设默认服 + WARN（绝不取首个在线 bukkit）；home-zone 配了但该 zone 无默认入口 → 不设 + WARN；默认入口当前不在线 → 不设 + WARN；连续选不出 → WARN 去重只打一次。
...
## 6. 风险 / 待定
- **BC↔zone 取舍**：BC 用自身 config `home-zone` 选默认入口是否触碰 ADR-0004「agent 不声明 zone」。本规格的立场（已被用户接受）：`home-zone` 是数据面路由配置（「我服务哪个 zone 的玩家」），非 serverId→zone 权威归属声明，默认入口值本身仍 DB 权威，故不违反。备选见 ADR-0031 备选方案。
- 多 BC 同 home-zone 共享同一默认入口 = 直接支持（各 BC 独立读发现、独立设自己的默认服）。
- 默认入口指向的服掉线：发现只返回 online+degraded，掉线后该实例不在发现集合 → agent 下一轮 sync 选不出 → 不设默认服 + WARN（不静默改落别的服，也不长期卡死在不可达默认服）。运维需另指定在线的默认入口。
