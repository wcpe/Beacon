...
- 并发发布同一 item 的串行化：「先软删后建」在并发下会撞 `uk_gray_item` 而瞬时死锁（InnoDB 间隙锁循环）。以 `config_item.gray_version` 乐观锁解决——发布前以其做 CAS（仅当未被并发改动才 +1，命中者才进「软删+建」段），CAS 在 item 行上串行化（单行锁、无环），从源头消除死锁；未命中者重读版本重试，各路最终都成功、恰留一条活跃灰度（重发即覆盖、后写赢）。不用 `FOR UPDATE`（快测 sqlite 不支持），不靠捕获死锁重试（治标不治本）。
