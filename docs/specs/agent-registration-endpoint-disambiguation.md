...
```
请求新路径
  ├─ 响应不是本控制面的 JSON（对端为老控制面，缺该路由）→ 改用旧路径重试一次
  └─ 是本控制面的 JSON → 按状态码既有映射处理，不回退
```

**判据必须看响应内容，不能只看状态码 404**（本项经真机实测修正）：

老控制面把未匹配路径交给内嵌前端做 SPA 兜底（`r.NotFound(h.Web.ServeHTTP)`），因此**有前端产物的正式构建会回 `200 + text/html` 而非 404**。真机实测三种形态：

| 对端 | 新路径响应 | 只看 404 的后果 |
|---|---|---|
| 新控制面 | `200 + application/json` | 正确（不回退） |
| 老控制面（无前端产物） | `404 + text/plain` | 正确（回退） |
| **老控制面（有前端产物）** | **`200 + text/html`** | ❌ 不回退 → 把 HTML 当 JSON 解析 |

故判据改为「响应 `Content-Type` 是否为本控制面的 `application/json`」：前两种形态都能正确回退，且 400 / 401 / 403 / 409 等**真实业务错误应答一律为 JSON**，绝不回退（否则 409 重复 serverId 会被误当作版本不匹配而重试）。

配套两条加固：

- **200 必须含非空 `instanceKey`**：它是挂载成功的必要事实（服务端恒回 namespace/serverId 拼成的实例键）。为空说明该 200 不是本端点的业务应答（被中间层 / SPA 兜底接管），绝不能当作成功——否则插件带着「已注册」的假象进入数据面，而控制面侧从未收到该实例。
- **解析异常兜底**：响应体非 JSON 时 `parseToJsonElement` 会抛异常，须在注册路径兜底转为 `Failed` 交上层退避重试，**绝不让解析异常逃逸打断注册循环**。
...
| 插件回退 | 新路径响应非本控制面 JSON 时插件自动用旧路径并成功（含老控制面 `404 + text/plain` 与 `200 + text/html` 两种形态）；为本控制面 JSON 时按状态码映射、不回退 |
| 假成功拒绝 | 新路径返回 200 但 `instanceKey` 为空（非本端点应答）时判为 `Failed`，不得当作注册成功 |
| 解析兜底 | 响应体非 JSON 致解析异常时转 `Failed` 交上层退避，异常不逃逸打断注册循环 |
