...
3. **三格式各自无损**：
   ...
   - **properties**：行式模型，保留每个 key 的前置注释行与原值文本，按 key 合并（override 替值、`null` 删键）、确定性键序输出、注释随键。不引新库。
