...
**状态**：部分已接受 / 部分被取代——其中「设置区聚合页（决策 3）+ 旧页折叠进设置子 tab（决策 7）+ 二级子 tab（决策 4/5 的子 tab 呈现）」部分已被 [ADR-0048](0048-flatten-system-nav-pages.md) 取代（系统区改为扁平独立页）；**「5 组手风琴侧栏导航（决策 1）+ NAV 单一真源收敛（决策 2）」仍有效保留**。
