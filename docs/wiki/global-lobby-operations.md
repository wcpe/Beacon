...
5. 为每台待确认身份填写审批原因并申请分配唯一 serverId；由另一位 human 在审批中心批准，等待 worker 执行。
...
在线服务的迁移受 drain 门保护；启用排空是强审计止损动作，取消排空和拓扑改派会创建审批申请。若管理台拒绝移动，先按既有运维流程排空或处理玩家，再重试。不要通过直接修改数据库绕过归属约束。
