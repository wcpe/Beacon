...
| FR-58 | 反向抓取受管任务 + 状态机（增强 FR-39）：抓取改为受管任务（状态机 扫描→待审→抓取→入库→done/failed/取消/过期 + 进度），单实例同时仅一个活跃任务（互斥），两段式命令（scan 返全树清单不传内容 / submit 选定回传）；超限文件不再整批失败。需写新 ADR 取代 [ADR-0027](adr/0027-reverse-fetch-channel-and-security.md) 决策5、扩展决策1 | P2 | 计划 |
| FR-59 | 反向抓取审核 · 忽略规则 · 冲突确认（增强 FR-39，依赖 FR-58）：扫描清单暂存；逐项/目录忽略（临时 + 可选存持久规则，按 namespace/组/实例）；提交选定集 ingest；入库前目标已有版本则 diff 确认保留哪份（复用 FR-46 范式）；忽略/确认/提交入审计 | P2 | 计划 |
| FR-60 | 反向抓取审核台 + 任务台（增强 FR-46，依赖 FR-58/59）：审核清单 UI（全量列 + 大小 + 超阈值红标须确认 + 逐项/目录忽略 + 实时 + 保存规则 + 提交）+ 任务查询/进度/状态机 + 冲突 diff 确认 | P2 | 计划 |
| FR-61 | 运维设置 store + 热生效（feat）：DB 设置表 + 设置 API（full 角色 + 审计）+ 各热改消费者从 store 读并 onChange 热生效（健康 TTL/间隔、采样间隔/保留、长轮询上限、日志级别、告警 webhook、反向抓取单文件上限…）；config.yml 降为引导 + 首启 seed，启动/安全项（http-addr/db/auth/agent-token）仍留文件。需写新 ADR 扩展 FR-25「config.yml 即真源」 | P2 | 计划 |
| FR-62 | 运维设置页（feat，依赖 FR-61）：分组展示热改项 + 编辑 + 保存 + 校验 + 热生效回显，标注「部分启动/安全项在 config.yml」 | P2 | 计划 |
| FR-63 | 可观测看板环境筛选可清空（fix）：选环境后无法清回「全部聚合」，修为可一键清回（可并入 FR-64） | P2 | 计划 |
| FR-64 | 可观测看板瘦身为「集群粗览」（增强 FR-32/FR-43）：紧凑、一屏不深滚，定位粗略系统运行总览（在线实例分角色/总人数/集群均值/健康分布 + 关键趋势缩略），逐服深数据移交 FR-65 | P2 | 计划 |
| FR-65 | 服务器详情页 = 代理服管理 + 实例与健康合并（增强 FR-52 + FR-5/FR-33/FR-49）：「代理服管理」改名 → 统一「服务器」页，列全部服务器(bukkit+bungee) 状态 + 角色 + 深指标(bukkit TPS/内存/CPU；bungee 连接/线程/运行时长/后端可达性) + 健康 + 下线/drain/zone 操作，点开看单服深详情 | P2 | 计划 |
