...
| FR-99 | 更新检查/状态/触发 API（feat，依赖 FR-97/FR-98，沿用预留 ADR-0044）：`/admin/v1` 下新增只读检查端点（当前版本/渠道/有无可用更新/可用版本/release 日志/发布时间/URL，服务端内存缓存 + `?force` 绕缓存 + GitHub 不可达 check-failed 降级不报 5xx）+ 状态端点（读内存态进度）+ 触发端点（POST 应用更新，写、readonly 403、入审计）；current=='dev' 不提示。一份端点契约真源（FR-100 消费） | P2 | 计划 |
