...
| FR-160 | 配置中心 V2 权威模型：基于 namespace / BC / 大区 / 小区 / 子服的配置作用域与继承关系 | P7 | 0.27.x | 配置作用域与区服权威模型一致；能解释每个配置值来自哪一层；跨 namespace 配置不可串用 | 开发中 |
| FR-161 | 配置中心 V2 编辑、校验、差异、版本与回滚 | P7 | 0.27.x | 支持结构化编辑、schema 校验、diff、版本历史和一键回滚；敏感值不泄露到日志与前端明文历史 | 开发中 |
