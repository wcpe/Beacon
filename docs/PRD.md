...
| FR-95 | 旧页折叠进设置页（增强 FR-82/FR-42/FR-53，依赖 FR-94，沿用预留 ADR-0043）：控制面健康(`/system`)/密钥管理(`/api-keys`)/环境管理(`/namespaces`) 三页作子 tab 内容复用嵌入（降内部重复标题、非激活子 tab 卸载停轮询），旧路由前端 Navigate 重定向到对应子 tab 深链，命令面板目标改指新深链。纯前端 | P2 | 计划 |
