...
| FR-93 | 侧栏导航层级化（增强 FR-6/FR-21，预留 ADR-0043 管理台导航分组与设置区聚合 IA）：15 平铺导航项收为 5 组可折叠手风琴（概览/配置管理/集群/可观测/系统），命中路由的组自动展开、展开态持久化到前端偏好；Layout NAV_ITEMS 与命令面板导航目标收敛为单一真源。纯前端 | P2 | 计划 |
| FR-94 | 设置区聚合页骨架（增强 FR-62/FR-77，沿用预留 ADR-0043）：`/settings` 升级为聚合页——三块顶层 tab（运维设置/系统信息/系统设置）+ 块内子 tab，嵌套子路由 + 子 tab search param 深链（刷新保持/后退/命令面板可检索）；运维设置 6 域改子 tab 呈现但**保留跨子 tab 统一草稿/批量保存/恢复默认**（不回归 FR-62/77）；一屏不滚动版式（tab 栏常驻、保存条 sticky、局部滚动）；预留「版本与更新/网络代理/更新设置」空壳子 tab 容器。纯前端 | P2 | 计划 |
