...
| FR-120 | 控制面手动回滚 API + 前端版本页按钮（feat，依赖 FR-119）：后端 POST /admin/v1/system/rollback（复用 FR-119 swap：运行版 → .failed、.old → 运行路径、spawn、退出；无 .old 返 409；readonly 403；触发入审计 system.update-rollback）；前端「版本与更新」页（/system/version）加「回滚到上一版本」按钮 + 二次确认 + 重连回显（FR-78）。验收：有 .old 回滚成功 / 版本回退 / 审计落库、无 .old 报错不误操作、readonly 403、cd web && pnpm test + build 绿、真机回滚一次过 | P2 | 开发中 |
