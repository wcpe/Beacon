...
| FR-100 | 页眉版本更新提示 + 更新模态框（feat，依赖 FR-99）：SystemHeader 版本徽章在有可用更新时叠小红点，点击弹更新模态框（当前/可用版本、渠道、release 日志安全渲染、发布时间、release 外链、更新按钮带二次确认+重启提示）；更新检查走独立低频 query（非 5s，与服务端缓存 TTL 对齐）+ 模态内「立即检查」；应用后轮询进度、复用 FR-78 连接指示重启后自动重连回显新版本。纯前端 | P2 | 计划 |
