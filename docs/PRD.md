...
| FR-178 | env 映射体验：env 增删改与 env→namespace 映射管理台 | P8 | 0.28.x | env 增删改、整体替换 env→namespace 映射与审计已落地；页眉 env 过滤当前仍非权威观测范围，必须由 FR-213/214 补齐服务端作用域与 env→namespace 级联后再验收。前端当前仅作 fail-closed 临时收窄：失效 scope 停查、多 namespace 逐个受限请求，不得回退全量 | 开发中（观测范围待归真） |
