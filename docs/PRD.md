...
| FR-137 | 管理台中间件高密度运维视觉重构（feat，纯前端，增强 FR-64/106/107/133/134）：按已确认的简洁中间件后台设计稿重做首页与核心页面视觉组织，覆盖 `/dashboard`、`/file-sync`、`/servers`、`/zones`、`/topology` 五个入口，形成高密度信息卡片 + 紧凑工具栏 + 裸表格 / 细线分区的一屏运维控制台；禁止回到 Minecraft 地图、像素风、NOC 大屏或游戏化装饰。`/file-sync` 必须优化 mock 1000+ 服务器的搜索、批量选择、剔除和已选摘要，避免超长 checkbox 列表不可操作。纯前端，不改后端 API / agent / SSE / 数据模型；通用无业务依赖组件进入 `web/packages/ui` 时同步 ui-wiki。验收：五页符合设计稿方向且业务行为不回归、1000+ 目标选择可操作、i18n 无缺键、无新增依赖、`cd web && pnpm test` + build 绿、1920×1080 浏览器 100% 缩放截图验收通过（见 [docs/specs/middleware-console-density-redesign.md](specs/middleware-console-density-redesign.md)） | P2 | 开发中 |
