...
| FR-146 | 调度决策记录：请求、候选、排除原因、最终选择、失败原因 | P4 | 0.24.x | 每次调度有 trace；可解释为什么选择 / 不选择某台服；失败可排查 | 开发中 |
| FR-147 | 健康值模型：TPS、CPU、连接、告警、容量、延迟综合评分 | P4 | 0.24.x | 每台服务器输出健康分、等级、不可调度原因；权重可配置并可回放解释 | 开发中 |
