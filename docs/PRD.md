...
| FR-128 | 配置工作台写操作接真后端（feat，补齐 FR-111 承诺、退场 FR-114 mock 桩）：双面板工作台此前所有写操作（新建 / 反向抓取 / 下发发布 / 删除）均为纯前端 mock toast（FR-114 原型留桩），仅读链路（受管树 listFiles / 服务器树 browse / 同步队列 / 操作日志）接真后端。本条把写操作全部接到既有真实端点——① 新建配置 → `createFile`（POST /files，覆盖层由当前 scope chip 推导，path 相对 plugins/ 根、自动去 `plugins/` 前缀防树内双层）；② 反向抓取 → `createScanTask` 建受管任务 + 审核浮层轮询真扫描清单 + `submitReverseFetchTask` 提交选定 + 冲突 `resolveConflicts`（复用 FR-58~60）；③ 发布选中 → `getFile`+`publishFile`（PUT /files/{id}，触发区内在线服秒级热更，复用 FR-2）；④ 删除 → `deleteFile`（DELETE /files/{id}）；写成功统一失效 `wb-managed-tree`/`wb-sync-queue` 重拉真相，mutation onError 脱敏 toast（复用 FR-122）。导入到组留 P2 待接。同步队列行名由 resultDetail 原始 JSON 改类型友好中文标签（反向抓取 / 浏览文件 / 热更下发 @ 实例）。验收：四类写操作调真端点、左面板真实刷新、新建不双层、队列友好标签、`cd web && pnpm test` + build 绿、真机 新建端到端 + 反抓扫描往返过（ingest / 热更后半程需稳定 agent） | P2 | 计划 |
| FR-134 | 管理台通用 UI 组件包化（refactor，增强 ADR-0012，需新 ADR）：将 `web/src/components` 中不绑定 Beacon 业务 API / 路由 / auth / 环境状态的通用 UI 与展示组件抽到内部包 `web/packages/ui`，主后台 Web 通过包名消费；`Layout` / `SystemHeader` / `CommandPalette` / `EnvSelector` / `useMessage` 等业务壳组件留在主应用。验收：UI 包可独立类型检查 / 构建，导出被抽出的通用组件；主后台现有路由与交互不回归；包内不得依赖 Beacon 业务 API、路由、auth、环境状态；主 Web `vitest`、`tsc -b`、`vite build` 绿 | P2 | 开发中 |
| FR-135 | UI 控件博物馆 wiki 子项目（feat，净新，依赖 FR-134，需 spec）：新增独立开发子项目 `web/apps/ui-wiki`，展示 `web/packages/ui` 的全部导出组件；每个组件至少展示基础用法、主要 variants / sizes / states，不随 Beacon 管理台生产包发布，不新增主后台 `/ui-wiki` 路由。验收：wiki 可独立启动 / 构建，覆盖 UI 包全部导出组件，示例不依赖 Beacon 后端；不引入 Storybook 等新依赖，复用现有 React / Vite / Tailwind / Radix 栈 | P2 | 开发中 |
| FR-136 | UI 包与控件博物馆文档（docs，依赖 FR-134/135）：补充内部 UI 包使用方式、组件导出规范、wiki 启动方式和新增控件展示约束，说明业务绑定组件仍留在主应用。验收：文档能指导开发者新增 / 引用 UI 包组件并在 wiki 中补展示；明确构建 / 测试命令；不把过程性计划文档入库 | P2 | 开发中 |
| FR-137 | 管理台中间件高密度运维视觉重构（feat，增强 FR-64/106/107/133/134）：按已确认的简洁中间件后台设计稿重做首页与核心页面视觉组织，覆盖 `/dashboard`、`/file-sync`、`/servers`、`/zones`、`/topology` 五个入口，形成高密度信息卡片 + 紧凑工具栏 + 裸表格 / 细线分区的一屏运维控制台；禁止回到 Minecraft 地图、像素风、NOC 大屏或游戏化装饰。`/file-sync` 必须优化 mock 1000+ 服务器的搜索、批量选择、剔除和已选摘要，避免超长 checkbox 列表不可操作。`/servers` 冻结补充复用既有下线 / 排空 / 重同步 / 日志 / 文件浏览 / 命令观测端点，补齐详情模态框、写操作确认与真实状态刷新，不新增后端契约 / agent / SSE / 数据模型；通用无业务依赖组件进入 `web/packages/ui` 时同步 ui-wiki。验收：五页符合设计稿方向且业务行为不回归、1000+ 目标选择可操作、服务器页详情与操作闭环真实可用、i18n 无缺键、无新增依赖、`cd web && pnpm test` + build 绿、1920×1080 浏览器 100% 缩放截图验收通过（见 [docs/specs/middleware-console-density-redesign.md](specs/middleware-console-density-redesign.md)、[docs/specs/server-page-operations-freeze.md](specs/server-page-operations-freeze.md)） | P2 | 开发中 |
