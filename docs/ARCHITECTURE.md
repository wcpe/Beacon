...
| `agent_command` | server→agent 命令（FR-39 反向抓取）：`ingest-plugins` 待办 + 生命周期 + 载荷 | 见下；`type`/`status` 落 `VARCHAR`、`payload`/`result_detail` 落 `TEXT`，真源属 DB（需持久/可审计/跨 SSE 断连重拉，[ADR-0027](adr/0027-reverse-fetch-channel-and-security.md)） |
...
`agent_command` 关键字段（FR-39，[ADR-0027](adr/0027-reverse-fetch-channel-and-security.md)）：`id`、`namespace` + `server_id`（目标 agent，进查询索引）、`type`（`ingest-plugins`，落 `VARCHAR` + 应用层校验）、`payload`（命令载荷 JSON 文本落 `TEXT`：反向抓取记目标 `scope`/`group`/`target`）、`status`（`pending`/`fetched`/`done`/`failed`/`expired`，落 `VARCHAR`，经 **CAS 迁移**：建即 `pending`、agent 拉走转 `fetched`、ingest 成功 `done`、校验/落库失败 `failed`、超时清理 `expired`）、`result_detail`（结果摘要 JSON 文本，无敏感内容）、`operator`（触发者）。**全部基础类型 + `TEXT`**，无 `ENUM/SET/JSON` 列与方言专有 SQL（可切 Postgres）。命令**真源属 DB**（区别于注册/健康的内存事实——命令需持久、可审计、跨 SSE 断连重拉）；建命令与 `file.reverse-fetch` 审计同事务原子落地、**提交后才**经 SSE `command-pending` 唤醒目标 agent（agent 经 agent §10 拉详情、读 `plugins/` 文本回传 §11，控制面入库前再校验后复用 FR-38 `Import` 落覆盖）。
