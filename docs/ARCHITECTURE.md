...
| `agent_command` | server→agent 命令（FR-39 反向抓取 / FR-46 按需拓印 / FR-58 受管任务两段式 / FR-88 取日志）：`ingest-plugins` / `tail-logs` 待办 + 生命周期 + 载荷（FR-46 加 `mode=imprint`/`path`、`ready` 态与瞬态 `imprint_content`；FR-58 加 `mode=scan`/`submit` + `selectedPaths`；FR-88 加 `tail-logs` 类型与瞬态 `log_content`） | 见下；`type`/`status` 落 `VARCHAR`、`payload`/`result_detail`/`imprint_content`/`log_content` 落 `TEXT`，真源属 DB（需持久/可审计/跨 SSE 断连重拉，[ADR-0027](adr/0027-reverse-fetch-channel-and-security.md) / [ADR-0040](adr/0040-agent-readonly-log-tail.md)） |
