...
| `agent_command` | server→agent 命令（FR-39 反向抓取 / FR-46 按需拓印 / FR-58 受管任务两段式）：`ingest-plugins` 待办 + 生命周期 + 载荷（FR-46 加 `mode=imprint`/`path`、`ready` 态与瞬态 `imprint_content`；FR-58 加 `mode=scan`/`submit` + `selectedPaths`） | 见下；`type`/`status` 落 `VARCHAR`、`payload`/`result_detail`/`imprint_content` 落 `TEXT`，真源属 DB（需持久/可审计/跨 SSE 断连重拉，[ADR-0027](adr/0027-reverse-fetch-channel-and-security.md)） |
