...
| `reverse_fetch_task` | 反向抓取受管任务（FR-58）：状态机 + 扫描清单 + 选定集 + 计数 + 命令引用，单实例互斥 | 见下；`status`/`scope` 落 `VARCHAR`、`manifest`/`selected_paths` 落 `TEXT`，活跃唯一以软删哨兵唯一键，真源属 DB（[ADR-0037](adr/0037-reverse-fetch-managed-task.md)） |
...
**软删唯一键**：`deleted_at` 默认值用**固定哨兵** `1970-01-01 00:00:00`（非 NULL）并纳入唯一键，软删时填真实时间——避免 NULL 不参与唯一比较导致"未删重复挡不住"，且 MySQL/Postgres 行为一致（见 [ADR-0008](adr/0008-config-soft-delete-and-effective-md5.md)）。`file_object` 同款哨兵软删。`reverse_fetch_task.active_at` 同款哨兵保活跃唯一（FR-58）。
