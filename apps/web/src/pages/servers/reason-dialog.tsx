// 带原因输入的写操作确认弹窗：承 UX §4「高风险二次确认 + 原因输入」。
// 影响预览 + 原因必填 + 提交进行中禁用 + 内联错误展示（ADR-0057，不静默隐藏）。
      ...
      <AlertDialogContent>
        ...
        {requireReason && (
          <div className="space-y-1.5">
            <Label htmlFor="reason-dialog-input">{t('cluster.servers.reason.label')}</Label>
            <Textarea
        ...
        <AlertDialogFooter>
          <AlertDialogCancel>{t('cluster.zones.create.cancel')}</AlertDialogCancel>
          <AlertDialogAction
            disabled={!canConfirm}
            onClick={(e) => {
