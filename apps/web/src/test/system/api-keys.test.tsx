// /api-keys 密钥页测试：常规渲染、空态引导、创建审批申请、吊销闭环。
...
describe('/api-keys 密钥页', () => {
  ...
  it('创建密钥后提交审批申请（写闭环）', async () => {
    useScenario('normal')
    server.use(
      http.post('/admin/v1/api-keys', async ({ request }) => {
        expect(request.headers.get('Idempotency-Key')).not.toBeNull()
        expect((await request.json()) as { reason?: string }).toMatchObject({ reason: '用于巡检自动化' })
        return HttpResponse.json(
          { approvalRequestId: 'apr_api_key_create', status: 'pending', operationKey: 'credential.create' },
          { status: 202 },
        )
      }),
    )
    ...
    await user.type(within(dialog).getByLabelText('审批原因'), '用于巡检自动化')
    await user.click(within(dialog).getByRole('button', { name: '创建' }))

    expect(await screen.findByText('API 密钥审批申请已创建，审批执行完成后可由原申请人一次性领取明文。')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: '前往审批中心查看进度' })).toHaveAttribute(
      'href',
      '/approvals?requestId=apr_api_key_create',
    )
  })
