...
func TestAllowMachineRegisterRequiresStrongAgentToken(t *testing.T) {
	...
	for name, token := range map[string]string{
		...
		".env.example 占位值": EnvExampleAgentToken,
		"留空":               "",
		"仅空白":              "   ",
	} {
...
// TestAllowMachineRegisterRejectsTokenCaseVariants 弱 token 判定须大小写不敏感：
// CHANGE-ME / Beacon-Bootstrap-Token 等变体不得绕过启动校验（只允许更严格，绝不放宽）。
func TestAllowMachineRegisterRejectsTokenCaseVariants(t *testing.T) {
	for _, token := range []string{
		"CHANGE-ME",
		"Change-Me",
		"BEACON-BOOTSTRAP-TOKEN",
		"Beacon-Bootstrap-Token",
		...
		cfg.MCP.AllowMachineRegister = true
		if err := cfg.validate(); err == nil {
			t.Fatalf("开关开启但 agent-token 为弱值的大小写变体 %q 时必须拒绝启动", token)
		}
	}
}
...
func TestMachineRegisterDefaultOffKeepsWeakTokenAccepted(t *testing.T) {
	for _, token := range []string{"", DefaultAgentToken, ExampleAgentToken, EnvExampleAgentToken, "any-token"} {
		cfg := validMCPConfigBase()
		cfg.AgentToken = token
