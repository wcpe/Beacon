...
// weakAgentTokens 是已知的弱 / 出厂默认 agent 共享 token（内置默认与配置样例默认）。
// 机器注册通道开启时持有该 token 等价于受信调用方，故这些值必须被拒绝。
var weakAgentTokens = map[string]struct{}{
	"":                {}, // 留空：无从比对，等于无门禁
	DefaultAgentToken: {}, // 内置默认（Default() / .env.example 的 change-me）
	ExampleAgentToken: {}, // 配置样例默认（config.example.yml 与 agent 样例开箱匹配值）
}

// isWeakAgentToken 判断 agent 共享 token 是否属于须拒绝的弱值（空白等价留空）。
func isWeakAgentToken(token string) bool {
	_, weak := weakAgentTokens[strings.TrimSpace(token)]
	return weak
}
