...
var (
	...
	// ErrReasonRequired 危险申请的提审入口缺少必填原因（申请人漏填）。
	// 与 ErrApprovalReasonRequired（"approval_reason_required"，审批人决策 / 拒绝时必填原因）是两个
	// 场景：前者失败在提审前，后者失败在对既有审批的决策上，调用方与处置方向都不同，故不复用同一码。
