...
func TestProxyURLValidation(t *testing.T) {
	...
	for _, v := range good {
		if err := applySettingUpdateForTest(svc, SettingUpdateProxyURL, v, "admin", ""); err != nil {
	...
	for _, v := range bad {
		if err := applySettingUpdateForTest(svc, SettingUpdateProxyURL, v, "admin", ""); err != apperr.ErrSettingValueInvalid {
...
func TestProxyURLAuditRedacted(t *testing.T) {
	...
	if err := applySettingUpdateForTest(svc, SettingUpdateProxyURL, "http://user:topsecret@proxy:8080", "alice", ""); err != nil {
...
func TestProxyURLUnchangedPlaceholderPreservesOriginal(t *testing.T) {
	...
	if err := applySettingUpdateForTest(svc, SettingUpdateProxyURL, "http://***:***@proxy:8080", "admin", ""); err != nil {
	...
	// 改成真正的新值 → 正常覆盖。
	if err := applySettingUpdateForTest(svc, SettingUpdateProxyURL, "http://user:newpass@proxy:8080", "admin", ""); err != nil {
