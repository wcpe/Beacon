...
func setupZoneAndAssign(t *testing.T, base, token string, nsID uint, guard harness.ProcessGuard) {
	...
	if _, err := harness.RequestApprovalAndWait(base, token, http.MethodPost, "/admin/v2/server-assignments", map[string]any{
...
func approveIdentity(t *testing.T, base, token, identityID string, guard *harness.GradleProc) {
	t.Helper()
	if err := harness.RequestApproveIdentityWithGuard(base, token, identityID, serverID, "调度 Agent E2E 确认身份", guard); err != nil {
